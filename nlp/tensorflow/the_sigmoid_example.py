...
def sigmoid_placeholder(W, b, session):
    '''
    The sigmoid example:
      h = sigmoid(W * x + b)
      sigmoid(x) = 1 / (1 + exp(-x))
    ...
    print('using placeholder x:', input)
    h_eval = session.run(
        h,
        feed_dict={x: input}  # feed_dict 参数给 placeholder x 赋值
    )
    print('result: %s\n' % h_eval)
...
def sigmoid_preloaded(W, b, session):
    ...
    print('preload using constant x:', x.eval().tolist())
...
def sigmoid_pipeline(W, b, session):
    ...
    # Coordinator 类可以视为 thread manager，管理使用多线程 enqueue，dequeue 等，
    # 管理多个 QueueRunners
    coord = tf.train.Coordinator()
    # 自动创建 QueueRunners，从定义好的 queue (filename_queue) 中获取数据，需要 explicitly 执行
    ...
    for step in range(5):
        x_eval, h_eval = session.run([x, h])  # 如果不指定 x，则 parameter server 不保存 x 结果，运算一样可以正常执行
        print('========== Step %d ==========' % step)
...
if __name__ == '__main__':
    ...
    print('\nCalculate sigmoid: h = sigmoid(x * W + b)')

    # variables: mutalbe tensors，算法中使用的变量，定义后需要执行 global_variables_initializer
    W = tf.Variable(
        tf.random_uniform(
    ...
    print('W:\n%s' % W.eval())  # 变量初始化以后才可以使用
    print('b:%s\n' % b.eval().tolist())  # 变量初始化以后才可以使用

    sigmoid_placeholder(W, b, session)
